...
interface IXtatuzReferral {
    function getProjectIdsByReferral(string memory referral_) external view returns (uint256[] memory);
    ...
    function buyerAgentDepositLeft(string memory referral, uint256 projectId_) external view returns (uint256);
    ...
    function increaseBuyerRef(
        ...
        uint256 referralAmount_
    ) external;

    function claim(string memory referral_, uint256 projectId_) external;

    function setDefaultPercentage(uint256 default_) external;

    function setMaxPercentage(uint256 default_) external;
    ...
    function withdrawFundsLeft(string memory referral_, uint256 projectId_) external;
